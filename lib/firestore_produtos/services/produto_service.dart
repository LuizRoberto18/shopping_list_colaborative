...
class ProdutoService {
  ...
  adicionarProduto({required String listinId, required Produto produto}) {
    firestore.collection(uid).doc(listinId).collection("produtos").doc(produto.id).set(produto.toMap());
  }

  Future<List<Produto>> lerProdutos(
      {required String listinId, required OrdemProduto ordem, required bool isDecrescente, QuerySnapshot<Map<String, dynamic>>? snapshot}) async {
    List<Produto> temp = [];
  ...
  alternarComprado({required Produto produto, required String listinId}) async {
    ...
    await firestore.collection(uid).doc(listinId).collection("produtos").doc(produto.id).update({"isComprado": produto.isComprado});
  }
  ...
  StreamSubscription<QuerySnapshot<Map<String, dynamic>>> conectarStream({
    ...
    return firestore.collection(uid).doc(listinId).collection("produtos").orderBy(ordem.name, descending: isDecrescente).snapshots().listen(
      (snapshot) {
  ...
  Future<void> removerProduto({required Produto produto, required String listinId}) async {
